...
public class CharacterSelect_Net : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        next.onClick.AddListener(delegate () { SceneManager.LoadScene("JoinGame"); });   //场景管理器，调用下一个场景
    ...
    void Update()
    {
        ...
        if (teamSelect.captionText.text.Equals("红"))
            OnlineSystem.Team = 0;
        else if(teamSelect.captionText.text.Equals("黑"))
            OnlineSystem.Team = 1;
    }
